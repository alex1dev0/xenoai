...
class Database {
    ...
    public function createConversation($title = 'Nuova Chat', $user_id = null) {
        $id = uniqid('chat_', true);
        $stmt = $this->pdo->prepare("INSERT INTO conversations (id, title, user_id) VALUES (:id, :title, :user_id)");
        $stmt->execute([':id' => $id, ':title' => $title, ':user_id' => $user_id]);
        return $id;
    }

    public function getConversations($user_id = null) {
        if ($user_id) {
            $stmt = $this->pdo->prepare("SELECT * FROM conversations WHERE user_id = :user_id ORDER BY updated_at DESC");
            $stmt->execute([':user_id' => $user_id]);
        } else {
            $stmt = $this->pdo->query("SELECT * FROM conversations WHERE user_id IS NULL ORDER BY updated_at DESC");
        }
